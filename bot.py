...
AVAILABLE_MODES = [("accept-edits", "Normal (Accept Edits)"), ("plan", "Plan Only")]
...
def get_main_menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="Effort", callback_data="open_effort_menu"),
...
async def cmd_menu(message: types.Message):
    ...
    session = get_session(user_id)
    text = (
        "<b>antigravity control panel</b>\n\n"
        f"model: <code>{session['model']}</code>\n"
        f"effort: <code>{session['effort']}</code>\n"
        f"mode: <code>{session['mode']}</code>\n"
        f"workspace: <code>{session['workspace']}</code>\n"
        f"session: <code>{session['conversation_id'] or 'new context'}</code>\n"
    )
...
async def cmd_resume(event: types.Message | types.CallbackQuery, command: Optional[CommandObject] = None):
    ...
    buttons = []
    for s in sessions:
        prefix = "* " if session["conversation_id"] == s["conv_id"] else ""
        btn_text = f"{prefix}[{s['mtime_str']}] {s['title']}"
    ...
    text = "<b>select session to resume:</b>"
...
async def cb_resume_conv(callback: types.CallbackQuery):
    ...
    await callback.answer("resuming session...")
...
async def cmd_model(message: types.Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=buttons)
    await track_and_answer(message.chat.id, user_id, f"<b>select model</b> (current: <code>{session['model']}</code>):", parse_mode="HTML", reply_markup=kb)
...
async def cb_open_model_menu(callback: types.CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=buttons)
    await safe_edit_text(callback.message, f"<b>select model</b> (current: <code>{session['model']}</code>):", reply_markup=kb, parse_mode="HTML")
...
async def cmd_effort(message: types.Message):
    ...
    await track_and_answer(message.chat.id, user_id, f"<b>select effort</b> (current: <code>{session['effort']}</code>):", parse_mode="HTML", reply_markup=kb)
...
async def cb_open_effort_menu(callback: types.CallbackQuery):
    ...
    buttons_row.append([InlineKeyboardButton(text="Back to Menu", callback_data="open_main_menu")])
    kb = InlineKeyboardMarkup(inline_keyboard=buttons_row)
    await safe_edit_text(callback.message, f"<b>select effort</b> (current: <code>{session['effort']}</code>):", reply_markup=kb, parse_mode="HTML")
...
async def cb_set_effort(callback: types.CallbackQuery):
    ...
    session["effort"] = eff
    logger.info(f"[EFFORT CHANGE] User {user_id} switched effort: {old_eff} -> {eff}")
...
async def cmd_mode(message: types.Message):
    ...
    buttons.append([InlineKeyboardButton(text="Back to Menu", callback_data="open_main_menu")])
    kb = InlineKeyboardMarkup(inline_keyboard=buttons)
    await track_and_answer(message.chat.id, user_id, f"<b>select mode</b> (current: <code>{session['mode']}</code>):", parse_mode="HTML", reply_markup=kb)
...
async def cb_open_mode_menu(callback: types.CallbackQuery):
    ...
    buttons.append([InlineKeyboardButton(text="Back to Menu", callback_data="open_main_menu")])
    kb = InlineKeyboardMarkup(inline_keyboard=buttons)
    await safe_edit_text(callback.message, f"<b>select mode</b> (current: <code>{session['mode']}</code>):", reply_markup=kb, parse_mode="HTML")
...
async def cb_set_mode(callback: types.CallbackQuery):
    ...
    logger.info(f"[MODE CHANGE] User {user_id} switched mode: {old_mode} -> {mode_id}")
...
async def cmd_cd(message: types.Message, command: CommandObject):
    ...
    if not command.args:
        await track_and_answer(message.chat.id, user_id, f"workspace: <code>{session['workspace']}</code>\nusage: <code>/cd /path/to/directory</code>", parse_mode="HTML")
        return
    ...
    await track_and_answer(message.chat.id, user_id, f"workspace updated: <code>{new_path}</code>", parse_mode="HTML")
...
async def cmd_status(event: types.Message | types.CallbackQuery):
    ...
    text = (
        ...
        f"mode: <code>{session['mode']}</code>\n"
        f"workspace: <code>{session['workspace']}</code>\n"
        f"session_id: <code>{session['conversation_id'] or 'none (new context)'}</code>\n"
        f"agy_binary: <code>{config.AGY_PATH}</code>\n"
    )
...
async def cmd_help(message: types.Message):
    ...
    text = (
        "<b>antigravity commands</b>\n\n"
        "• <code>/resume</code> - select and restore previous session\n"
        "• <code>/new</code> - clear chat and start fresh session\n"
        "• <code>/menu</code> - open control panel\n"
        "• <code>/model</code> - select AI model\n"
        "• <code>/effort</code> - set reasoning effort (low/medium/high)\n"
        "• <code>/mode</code> - set execution mode (accept-edits/plan)\n"
        "• <code>/cd /path</code> - change workspace directory\n"
        "• <code>/status</code> - view current configuration"
    )
...
async def cb_stop(callback: types.CallbackQuery):
    ...
    await safe_edit_text(callback.message, "task execution cancelled.", parse_mode="HTML")
...
async def handle_voice_message(message: types.Message):
    ...
    logger.info(f"[VOICE MESSAGE] User {user_id} sent voice message (file_id={message.voice.file_id})")
...
async def handle_text_prompt(message: types.Message):
    ...
    status_msg = await track_and_answer(message.chat.id, user_id, "<i>processing prompt...</i>", parse_mode="HTML", reply_markup=stop_kb)

    full_text = ""
    last_update_time = 0.0

    prompt_with_hint = prompt + (
        "\n\n[System Note for Telegram output:\n"
        "- Write clear, direct, and technical answers.\n"
        "- Do NOT use corporate AI fluff, fake sycophantic enthusiasm, or emoji spam.\n"
        "- Use standard fenced code blocks for code or shell commands.\n"
        "- Use blockquotes (`> text`) or bullet lists where appropriate.\n"
        "- Avoid markdown tables (use code blocks or list format).]"
    )
            ...
            for img_path in found_paths:
                if os.path.exists(img_path) and os.path.isfile(img_path):
                    try:
                        ...
                        img_msg = await bot.send_photo(message.chat.id, FSInputFile(img_path), caption=os.path.basename(img_path))
                        session["sent_message_ids"].append(img_msg.message_id)
                    except Exception as img_err:
                        logger.error(f"Failed to auto-send image {img_path}: {img_err}")
        else:
            await safe_edit_text(status_msg, "<i>done (no output)</i>", parse_mode="HTML")

    except Exception as e:
        logger.error(f"[UNHANDLED ERROR] Exception during prompt handling: {e}", exc_info=True)
        await safe_edit_text(status_msg, f"<b>Error</b>: <code>{html.escape(str(e))}</code>", parse_mode="HTML")
